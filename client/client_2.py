...
def main():
    ...
    parser.add_argument('-m', '--model', default=None, dest="model", help="model in azure_template container")
    ...
    if args.mode == 'file' or args.mode == 'stream':
        ...
        url = args.uri + '?%s' % (urllib.urlencode([("content-type", content_type)])) + '&%s' % (urllib.urlencode([("accessToken", args.token)])) + '&%s' % (urllib.urlencode([("model", args.model)]))
